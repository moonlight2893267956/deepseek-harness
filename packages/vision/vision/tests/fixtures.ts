/**
 * Test fixtures for the vision plugin.
 *
 * Builds frozen `UserMessage`s with image blocks and a fake `LlmRuntime` whose
 * `stream` emits a configurable description, so the core preprocessing can be
 * exercised without a real vision provider.
 */
...
/**
 * A fake `LlmRuntime` whose vision `stream` emits the supplied text as text-delta chunks.
 * Pass `undefined` to emulate a vision model that yields no text (forces failure).
 */
export function fakeLlmRuntime(text: string | undefined): LlmRuntime {
  ...
  // block-end carrying the full block; the vision consumer must not double-count.
...
/**
 * Wrap {@link fakeLlmRuntime} to capture the vision sub-call options (e.g. the
 * injected system prompt) for assertions.
 * @param text - the description to stream, or `undefined` to force failure.
 * @param onOptions - invoked with the sub-call options before the stream runs.
 */
export function spyLlmRuntime(
  text: string | undefined,
  onOptions?: (options: { provider: string; model: string; system?: string; maxTokens?: number }) => void,
): LlmRuntime {
...
/** A fake `LlmRuntime` that rejects, simulating a vision engine transport failure. */
